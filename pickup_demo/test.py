...
class Grocery_Robot{
	...
	public:
		...
		VectorXd HomePosition;//joint position to keep while traveling moving only platform (ignore first 3, that are of the platform.)
...
void Controller::Base_controller(Vector3d Position=VectorXd::Zero(3)){
	/**
	 * move base to the x,y,theta position while keeping the joints position
	...
	Whole_body_controller=- (kvj *dq) -kpj*(q-q_des);
...
int main() {
	...
	Robot.dof = robot->dof();
	Robot.HomePosition<<0.0,0.0,0.0,-0.07,-0.1,0.04,0.04,0.04,0.04,0.04,0.04,0.04;
...
fTimerDidSleep = timer.waitForNextLoop();
		...
		case Simulation_states::PICK_SHELF_OBJECTS:
			...
			case Robot_States::R_IDLE:
				...
				// // RobotController.Base_controller(Position);
				
				// //RobotController.Arm_Local_Position_Orientation_controller(Position);
				// RobotController.Arm_World_Position_Orientation_controller(Position);
				// control_torques=RobotController.Return_torques();
